@php View::share('pageTitle', 'Applications/Systems Management'); @endphp
<x-app-layout>
    <x-slot name="header" style="display:none;"></x-slot>

    <x-db2-shell
        title="Applications/Systems Management"
        subtitle="Manage available applications and systems."
    >
        <x-slot name="actions">
            <button
                type="button"
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="inline-flex gap-2">
                                    <button 
                                        ...
                                        onclick="openEditDialog({{ $system->id }}, '{{ addslashes($system->name) }}', {{ $system->is_active ? 'true' : 'false' }})"
                                    >
                                        Edit
                                    </button>
        ...
        <dialog id="create-system-dialog" class="w-full max-w-lg rounded-2xl border border-slate-200 p-0 backdrop:bg-slate-900/40">
            <div class="rounded-2xl bg-white p-6 sm:p-8">
                ...
                <form method="POST" action="{{ route('admin.application-systems.store') }}" class="mt-6 space-y-4">
                    ...
                    <div>
                        <label class="auth-label block text-sm font-medium text-slate-700" for="name">System Name</label>
                        <input class="auth-input mt-1 block w-full rounded-lg border-slate-300 shadow-sm focus:border-slate-500 focus:ring-slate-500 sm:text-sm" id="name" name="name" type="text" placeholder="e.g., Office Automation System, Document Management" required autofocus>
                    </div>
        ...
        <dialog id="edit-system-dialog" class="w-full max-w-lg rounded-2xl border border-slate-200 p-0 backdrop:bg-slate-900/40">
            <div class="rounded-2xl bg-white p-6 sm:p-8">
                ...
                <form id="edit-system-form" method="POST" action="" class="mt-6 space-y-4">
                    ...
                    <div>
                        <label class="auth-label block text-sm font-medium text-slate-700" for="edit_name">System Name</label>
