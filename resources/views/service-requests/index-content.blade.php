...
<div class="db2-page-head">
    <div>
        <h1 class="db2-page-title">{{ $isArchiveView ? 'Archive' : ($isReceivedView ? 'Receive' : ($isAssignedView ? 'Assigned' : 'Service Requests')) }}</h1>
        <p class="db2-page-subtitle">{{ $isArchiveView ? 'Approved request records.' : ($isReceivedView ? 'Requests you have received.' : ($isAssignedView ? 'Requests assigned to you.' : 'Track submitted DOH service request forms.')) }}</p>
    </div>
</div>
