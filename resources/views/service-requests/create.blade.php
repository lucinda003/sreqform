<x-app-layout>
    ...
    <x-slot name="header">
        <div>
            ...
            <p class="auth-subtitle">Fill in the details below to submit a DOH service request.</p>
        </div>
    </x-slot>
        ...
        <div class="rounded-2xl border border-white/70 bg-white/80 p-5 shadow-lg backdrop-blur-xl sm:p-8">
            <form method="POST" action="{{ route('service-requests.store') }}" class="space-y-6">
                ...
                <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                    ...
                    <p class="mt-2 text-sm text-slate-600">Reference code is auto-generated per department and request date.</p>
                    <div class="mt-3 grid gap-4 sm:grid-cols-2">
                        <div>
                            <label for="department_code" class="auth-label">Department Code</label>
                            <input id="department_code" name="department_code" type="text" class="auth-input" value="{{ old('department_code', $isAdmin ? '' : $userDepartment) }}" @readonly(! $isAdmin) required>
                        ...
                        <div>
                            ...
                            <input id="request_date" name="request_date" type="date" class="auth-input" value="{{ old('request_date', now()->toDateString()) }}" required>
                            <x-input-error :messages="$errors->get('request_date')" class="mt-2" />
                        </div>
                    </div>
                    <p class="mt-3 text-xs text-slate-500">Example format: DEPT-20260318-001</p>
                </div>
                ...
                <div>
                    ...
                    <div class="mt-3 grid gap-4 sm:grid-cols-3">
                        <div>
                            ...
                            <input id="contact_last_name" name="contact_last_name" type="text" class="auth-input" value="{{ old('contact_last_name') }}" required>
                        ...
                        <div>
                            ...
                            <input id="contact_first_name" name="contact_first_name" type="text" class="auth-input" value="{{ old('contact_first_name') }}" required>
                        ...
                        <div>
                            ...
                            <input id="contact_middle_name" name="contact_middle_name" type="text" class="auth-input" value="{{ old('contact_middle_name') }}">
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <input id="office" name="office" type="text" class="auth-input" value="{{ old('office') }}" required>
                        <x-input-error :messages="$errors->get('office')" class="mt-2" />
                    </div>
                    <div>
                        <label for="address" class="auth-label">Address</label>
                        <input id="address" name="address" type="text" class="auth-input" value="{{ old('address') }}" required>
                        <x-input-error :messages="$errors->get('address')" class="mt-2" />
                    </div>
                    <div>
                        <label for="landline" class="auth-label">Landline</label>
                        <input id="landline" name="landline" type="text" class="auth-input" value="{{ old('landline') }}">
                        <x-input-error :messages="$errors->get('landline')" class="mt-2" />
                    </div>
                    <div>
                        <label for="fax_no" class="auth-label">Fax No.</label>
                        <input id="fax_no" name="fax_no" type="text" class="auth-input" value="{{ old('fax_no') }}">
                        <x-input-error :messages="$errors->get('fax_no')" class="mt-2" />
                    </div>
                    <div class="sm:col-span-2">
                        <label for="mobile_no" class="auth-label">Mobile No.</label>
                        <input id="mobile_no" name="mobile_no" type="text" class="auth-input" value="{{ old('mobile_no') }}" required>
                ...
                <div>
                    ...
                    <textarea id="description_request" name="description_request" class="auth-input min-h-32" required>{{ old('description_request') }}</textarea>
                ...
                <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                    ...
                    <div class="mt-3 grid gap-4 sm:grid-cols-2">
                        <div>
                            ...
                            <input id="approved_by_name" name="approved_by_name" type="text" class="auth-input" value="{{ old('approved_by_name') }}" required>
                        ...
                        <div>
                            ...
                            <input id="approved_by_position" name="approved_by_position" type="text" class="auth-input" value="{{ old('approved_by_position') }}" required>
                        ...
                        <div>
                            ...
                            <input id="approved_date" name="approved_date" type="date" class="auth-input" value="{{ old('approved_date', now()->toDateString()) }}" required>
                ...
                <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                    ...
                    <div class="mt-3 grid gap-4 sm:grid-cols-2">
                        <div>
                            ...
                            <input id="kmits_date" name="kmits_date" type="date" class="auth-input" value="{{ old('kmits_date', now()->toDateString()) }}" required>
                            <x-input-error :messages="$errors->get('kmits_date')" class="mt-2" />
                        </div>
                        <div>
                            <label for="time_received" class="auth-label">11. Time Received</label>
                            <input id="time_received" name="time_received" type="time" class="auth-input" value="{{ old('time_received') }}">
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <textarea id="actions_taken" name="actions_taken" class="auth-input min-h-24">{{ old('actions_taken') }}</textarea>
                            ...
                            <tbody>
                                @for ($i = 0; $i < 5; $i++)
                                    <tr class="border-t border-slate-100">
                                        <td class="px-2 py-2"><input name="action_log_date[]" type="date" class="auth-input !min-h-0 py-2" value="{{ old('action_log_date.' . $i) }}"></td>
                                        <td class="px-2 py-2"><input name="action_log_time[]" type="time" class="auth-input !min-h-0 py-2" value="{{ old('action_log_time.' . $i) }}"></td>
                                        <td class="px-2 py-2"><input name="action_log_action_taken[]" type="text" class="auth-input !min-h-0 py-2" value="{{ old('action_log_action_taken.' . $i) }}"></td>
                                        <td class="px-2 py-2"><input name="action_log_action_officer[]" type="text" class="auth-input !min-h-0 py-2" value="{{ old('action_log_action_officer.' . $i) }}"></td>
                    ...
                    <div class="mt-4 grid gap-4 sm:grid-cols-3">
                        <div>
                            ...
                            <input id="noted_by_name" name="noted_by_name" type="text" class="auth-input" value="{{ old('noted_by_name') }}">
                            <x-input-error :messages="$errors->get('noted_by_name')" class="mt-2" />
                        </div>
                        <div>
                            <label for="noted_by_position" class="auth-label">14. Position</label>
                            <input id="noted_by_position" name="noted_by_position" type="text" class="auth-input" value="{{ old('noted_by_position') }}">
                        ...
                        <div>
                            ...
                            <input id="noted_by_date_signed" name="noted_by_date_signed" type="date" class="auth-input" value="{{ old('noted_by_date_signed') }}">
                ...
                <div class="flex flex-wrap items-center gap-3">
                    ...
                    <a href="{{ route('service-requests.index') }}" class="rounded-xl border border-slate-300 px-4 py-2.5 text-sm font-medium text-slate-700 transition hover:border-slate-500 hover:text-slate-900">View Submitted Requests</a>
                </div>
            </form>
        </div>
    </div>
